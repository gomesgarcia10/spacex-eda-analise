...
```python
# Gráfico de dispersão cruzando número do voo, massa da carga e o resultado do pouso
sns.scatterplot(data=df, x='FlightNumber', y='PayloadMass', hue='Class', palette={0: '#e74c3c', 1: '#2ecc71'}, s=100)
```
\
...
```python
# Taxa de sucesso filtrada por tipo de órbita
sucesso_orbita = df.groupby('Orbit')['Class'].mean().sort_values(ascending=False) * 100
sns.barplot(x=sucesso_orbita.index, y=sucesso_orbita.values, hue=sucesso_orbita.index, palette="viridis", legend=False)
```
\
...
```python
# Eficiência de pouso por localização geográfica de lançamento
sucesso_base = df.groupby('LaunchSite')['Class'].mean().sort_values(ascending=False) * 100
sns.barplot(x=sucesso_base.index, y=sucesso_base.values, hue=sucesso_base.index, palette="magma", legend=False)
```
\
